...
## 7a · Umsetzungsstand (bis 2026-09-19 — K1–K10, O6, K4b und Kassensturz S5)
...
**Kassensturz S5 — der Verlauf (2026-09-19, auf Fabios „mach das alles“):**
1. *EIN Schreibweg in den Verlauf (K4):* „Meiner gilt“ (Basis heben), „Verwerfen“, „Übertragen“, „Zuordnen“ (Rebase) und das Rückgängig einer Version gehen in die OFFENE Bearbeitung und werden mit ihr unter der Beschreibung des Nutzers versioniert (`useAenderungen._inDenVerlauf`); läuft keine, wird die Entscheidung sofort eine Version wie bisher. Bis hierher wurde jede davon ihre eigene Version, auch mitten in einer Bearbeitung — zwei Wege in den Verlauf, der zweite an der Beschreibung vorbei. Direkt über `_commitAus` versionieren nur noch, was keine Bearbeitung hat, in die es gehört: Rückgängig/Wiederholen einer Version über die Pfeile, die Übernahmen beim Laden. Gemessen an den fünf Entscheidungen bei offener Bearbeitung: vorher je +1 Version, jetzt 0 und die Schritte in der Bearbeitung.
2. *„+ Satz“ als Kopie (K2):* statt eines `prompt` ein Dialog (`components/SatzNeuDialog.vue`): „Kopie von ‹Satz›“ (dieselben Modelle und der ganze Verlauf, vorgewählt, wenn der Satz einen hat) oder „Neu beginnen“ (dieselben Modelle, ohne Verlauf). Die Kopie ist `useAenderungen.kopiereSatz(von, nach)` über die `stand:<id>`-Bereiche — frisch vom Server gelesen, nie über einen vorhandenen Verlauf, ohne den Schreibstand des Mehrbenutzer-Wächters (der neue Satz beginnt seine eigene Schreibfolge). Ein Satzwechsel allein kopiert weiter nichts. Wie beim Wechsel ist „Neuer Satz“ bei offener Bearbeitung gesperrt (der Sichern-Dialog geht auf) — sonst stünde der Entwurf im alten Satz oder, kopiert, in beiden.
Tests `schreibweg.test.js` (13) und `satzNeu.test.js` (4, Dialog montiert, echte Stores); gegen den alten Store 7 von 13 rot, Dialog-Gegenproben 4 von 4 rot; `rebase.test.js` und `sitzung.test.js` gedreht (Entscheidungen bei offener Bearbeitung). Im Browser (:3001, 42069, echter Klick im Satz-Menü): Kopie vorgewählt mit „1 Version“, der neue Satz lädt mit derselben Version und demselben Wert, die Serverdatei trägt 1 Schritt und keinen fremden Schreibstand; ein Schritt im neuen Satz lässt den alten stehen; „Neuer Satz“ bei offener Bearbeitung öffnet „Bearbeitung sichern“; Rückgängig einer Version bei offener Bearbeitung: Versionen 1 → 1, Schritte 1 → 2, nach dem Sichern 2 Versionen. Aufgeräumt ([Bild](bilder/s5-satz-kopie.png)). Client 442 Dateien / 4 715 Tests grün.
**Nebenbefund, nicht angefasst:** das Rückgängig einer Version bei offener Bearbeitung überschreibt spätere Arbeit DERSELBEN Bearbeitung am selben Bauteil (der Gegeneintrag ist der neueste — dieselbe Regel, nach der Rückgängig nur an der neuesten Version angeboten wird). Das war vorher genauso; jetzt steht es wenigstens als Schritt in der Bearbeitung und lässt sich dort herausnehmen.
