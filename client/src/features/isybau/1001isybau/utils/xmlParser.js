...
const parseEdge = (obj, id) => {
    ...
    if (geom) {
        ...
        if (polyKantes.length > 0) {
            ...
            for (let i = 0; i < polyKantes.length; i++) {
                ...
                const start = kante.getElementsByTagName("Start")[0];
                const end = kante.getElementsByTagName("Ende")[0];

                if (start) {
                    // Only add start point if it's the first segment or not duplicate of previous end
                    // Simple approach: Add start of first, then end of all
                    if (i === 0) {
                        coords.push({
                            ...
                            z: parseFloat(start.getElementsByTagName("Punkthoehe")[0]?.textContent)
                        });
                    }
                }
                if (end) {
                    coords.push({
                        x: parseFloat(end.getElementsByTagName("Rechtswert")[0]?.textContent),
                        y: parseFloat(end.getElementsByTagName("Hochwert")[0]?.textContent),
                        z: parseFloat(end.getElementsByTagName("Punkthoehe")[0]?.textContent)
                    });
                }
            }
        }
    ...
    const status = obj.getElementsByTagName("Status")[0]?.textContent;
