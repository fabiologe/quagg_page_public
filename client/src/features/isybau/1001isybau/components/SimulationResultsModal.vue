            ...
            <table v-else>
              ...
              <tbody>
                <tr v-for="edge in sortedEdges" :key="edge.id">
                  ...
                  <td>
                    <button @click="selectEdge(edge.id)" class="secondary-btn btn-sm">Details</button>
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <table v-else>
              ...
              <tbody>
                <tr v-for="node in sortedNodes" :key="node.id">
                  ...
                  <td>
                    <button @click="selectNode(node.id)" class="secondary-btn btn-sm">Details</button>
                  </td>
                </tr>
              </tbody>
            </table>
...
const getNodeTypeLabel = (type) => {
  if ([2, 3, 4, 12, 13].includes(type)) return getMapping('Bauwerkstyp', type) || 'Bauwerk';
  return 'Schacht';
};
...
const sortedNodes = computed(() => {
  return Array.from(props.nodes.values()).sort((a, b) => {
    // Sort Bauwerke first
    const isBauwerkA = [2, 3, 4, 12, 13].includes(a.type);
    const isBauwerkB = [2, 3, 4, 12, 13].includes(b.type);
    if (isBauwerkA && !isBauwerkB) return -1;
    if (!isBauwerkA && isBauwerkB) return 1;
