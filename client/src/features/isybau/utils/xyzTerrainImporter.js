 ...
 *
 * DGM-Import für isybau: lädt ein vom Nutzer hochgeladenes Höhenraster (XYZ-
 * Punktwolke ODER echtes ESRI-ASCII-Grid) zu einem regulären DGM-Raster.
 * Portiert von flood-2D/middleware/importers/xyzTerrainImporter.js (dort für
 * die Bathymetrie/Gelände-Grundlage der 2D-Flutsimulation gebaut) — reine,
 * Vue/Store-freie Funktionen, deshalb 1:1 übertragbar.
 *
 * Zwei unterstützte Eingabeformate:
 *   1. ESRI ASCII Grid (Header ncols/nrows/cellsize/xllcorner|xllcenter/
 *      yllcorner|yllcenter/nodata_value + Matrix-Body) — das Format, in dem
 *      amtliche deutsche DGM1-Downloads überwiegend vorliegen. Schon ein
 *      reguläres Raster, braucht keine Interpolation.
 *   2. XYZ/TXT-Punktwolke (X Y Z pro Zeile, regulär oder irregulär gestreut)
 *      — Original-Pfad aus flood-2D, TIN (Delaunay) + IDW-Fallback.
 *
 * Kernproblem der Alt-Logik (Punktwolken-Pfad): Die Zellweite wurde aus dem
 * Abstand der ersten zwei X-Werte abgeleitet. Bei unregelmäßigen Daten ist
 * dieser Abstand winzig → ncols*nrows explodiert → `new Float32Array(...)`
 * wirft `RangeError: Invalid array length`.
 *
 * Dieses Modul:
 *   0. parseEsriAsciiGrid() – erkennt+liest ein echtes ESRI-ASCII-Grid direkt.
 *   1. parseXyzPoints()    – kanonische, speichereffiziente Parse-Logik für Punktwolken.
 ...
 *   4. makeTerrainObject() – kanonische Terrain-Form für store.importTerrain().
 *
 * Konventionen (identisch zum flood-2D-Original):
...
/**
 * Zahl parsen. Komma als Dezimaltrennzeichen wird bewusst NICHT unterstützt —
 * der Tokenizer spaltet Zeilen u.a. an Kommas (für CSV-artige Punktlisten),
 * das lässt sich mit „Komma als Dezimaltrennzeichen" nicht widerspruchsfrei
 * kombinieren. ESRI-ASCII-Grid ist ein maschinell erzeugtes, nicht lokalisiertes
 * Format — Dezimalpunkt ist dort die durchgängige Konvention. Falls eine reale
 * DGM1-Datei doch Komma-Dezimalzahlen liefert, hier gezielt nachbessern.
 */
function parseNum(tok) {
    return parseFloat(tok);
}

// ─── ESRI ASCII Grid (amtliches DGM1-Standardformat) ───────────────────────

const ESRI_HEADER_KEYS = new Set([
    'ncols', 'nrows', 'cellsize', 'xllcorner', 'yllcorner', 'xllcenter', 'yllcenter', 'nodata_value',
]);

/**
 * Erkennt+liest ein ESRI-ASCII-Grid (Header + Zeilen-Matrix aus Z-Werten,
 * Datei-Zeile 0 = Norden/oben). Gibt `null` zurück, wenn der Text keinen
 * ESRI-Header hat (dann versucht der Aufrufer stattdessen parseXyzPoints).
 *
 * @param {string} text
 * @returns {{data: Float32Array, header: {ncols:number,nrows:number,cellsize:number,xllcorner:number,yllcorner:number,xll:number,yll:number}}|null}
 */
export function parseEsriAsciiGrid(text) {
    ...
    while (i < lines.length) {
        ...
        if (!ESRI_HEADER_KEYS.has(key)) break;
        header[key] = parseNum(parts[1]);
        i++;
    }

    // ncols/nrows sind in jeder ESRI-Grid-Spezifikation obligatorisch und immer
    // die ersten Header-Zeilen — ihr Fehlen heißt "das ist kein ESRI-Grid".
    if (!(header.ncols > 0) || !(header.nrows > 0)) return null;

    const ncols = Math.round(header.ncols);
    const nrows = Math.round(header.nrows);
    if (ncols * nrows > MAX_CELLS) {
        throw new Error(`ESRI-ASCII-Grid zu groß (${ncols}×${nrows} Zellen, Limit ${MAX_CELLS.toLocaleString()}).`);
    }

    let cellsize = header.cellsize;
    if (!(cellsize > 0) || !isFinite(cellsize)) cellsize = 1;

    // xllcenter/yllcenter (Zellzentrum) ODER xllcorner/yllcorner (Zellecke) —
    // beide Varianten kommen in freier Wildbahn vor.
    let xll, yll;
    if (header.xllcenter !== undefined) xll = header.xllcenter;
    else if (header.xllcorner !== undefined) xll = header.xllcorner + cellsize / 2;
    else return null;
    if (header.yllcenter !== undefined) yll = header.yllcenter;
    else if (header.yllcorner !== undefined) yll = header.yllcorner + cellsize / 2;
    else return null;

    const nodataValue = header.nodata_value !== undefined ? header.nodata_value : NODATA;

    // Body einlesen: Werte werden über Zeilengrenzen hinweg gezählt statt eine
    // feste "1 Datei-Zeile = 1 Rasterzeile"-Annahme zu treffen — manche Exporte
    // brechen lange Zeilen um. Datei-Zeile 0 = Norden = TOP → gridData ist
    // bottom-up, also Zeilen-Flip beim Schreiben.
    const total = ncols * nrows;
    const data = new Float32Array(total).fill(NODATA);
    let filled = 0;
    for (; i < lines.length && filled < total; i++) {
        const l = lines[i].trim();
        if (!l) continue;
        const toks = l.split(/[\s,;]+/).filter(Boolean);
        for (let k = 0; k < toks.length && filled < total; k++) {
            const v = parseNum(toks[k]);
            const fileRow = Math.floor(filled / ncols);
            const col = filled % ncols;
            const row = (nrows - 1) - fileRow; // Norden(Datei-Zeile 0) -> oberste bottom-up-Zeile
            data[row * ncols + col] = (!isFinite(v) || v === nodataValue) ? NODATA : v;
            filled++;
        }
    }
    if (filled < total) {
        throw new Error(`ESRI-ASCII-Grid unvollständig: ${filled} von ${total} Werten gelesen.`);
    }
...
// ─── XYZ-Punktwolke (Fallback für nicht-gerasterte Daten) ──────────────────
...
export function parseXyzPoints(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (ESRI_HEADER_KEYS.has(first)) continue;
        const x = parseNum(parts[0]);
        const y = parseNum(parts[1]);
        const z = parseNum(parts[2]);
 ...
 * store.importTerrain() / useTerrainLayer.js erwarten.
 *
...
/**
 * Komfort-Einschritt: Text → kanonisches Terrain-Objekt. Versucht zuerst ein
 * echtes ESRI-ASCII-Grid zu lesen (kein TIN/IDW nötig, Datei ist schon
 * regulär); fällt sonst auf die XYZ-Punktwolke + TIN/IDW zurück.
...
export function importXyzTerrain(text, opts = {}) {
    const esri = parseEsriAsciiGrid(text);
    if (esri) {
        const { ncols, nrows, cellsize, xll, yll } = esri.header;
        return {
            terrain: makeTerrainObject(esri.data, esri.header),
            analysis: {
                count: ncols * nrows,
                isRegular: true,
                ...
                estRows: nrows,
                extent: {
                    minX: xll, maxX: xll + (ncols - 1) * cellsize,
                    minY: yll, maxY: yll + (nrows - 1) * cellsize,
                    width: (ncols - 1) * cellsize, height: (nrows - 1) * cellsize,
                },
    ...
    return { terrain: makeTerrainObject(data, header), analysis: { ...analysis, format: 'xyz-points' } };
}
