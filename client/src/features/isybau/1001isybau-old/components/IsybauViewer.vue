    ...
    <Transition name="slide-up">
      <div v-if="selectedElement" class="info-window">
        ...
        <div class="info-content">
            ...
            <template v-if="hydraulics && hydraulics.get(selectedElement.id)">
              ...
              <div class="info-row" v-if="hydraulics.get(selectedElement.id).utilization !== undefined">
                ...
                <span class="value" :style="{ color: hydraulics.get(selectedElement.id).utilization > 1 ? 'red' : 'inherit' }">
                  {{ (hydraulics.get(selectedElement.id).utilization * 100).toFixed(0) }} %
                </span>
              </div>
            </template>
          </template>
          ...
          <!-- Node Specifics -->
          <template v-else-if="selectedElement.type === 'Schacht' || selectedElement.type === 'Knoten'">
            <div class="info-row">
              <span class="label">Status:</span>
              <span class="value">{{ getMapping('Status', selectedElement.status) }}</span>
            </div>
            <div class="info-row">
              ...
              <input 
                ...
                @change="updateElement('z', $event.target.value)"
                step="0.01"
              >
            </div>
            <div class="info-row">
              ...
              <input 
                ...
                @change="updateElement('depth', $event.target.value)"
                step="0.01"
              >
            </div>
          </template>
          ...
          <template v-else-if="selectedElement.points">
            ...
            <div class="info-row" v-if="selectedElement.runoffCoeff !== undefined">
              ...
              <input 
                type="number" 
                class="edit-input" 
                :value="selectedElement.runoffCoeff" 
                @change="updateElement('runoffCoeff', $event.target.value)"
                step="0.01"
                min="0"
                max="1"
              >
...
import { getMapping } from '../utils/mappings';
...
const emit = defineEmits(['select-node', 'select-edge', 'update-element']);
...
const selectElement = (element, type) => {
  if (mode.value !== 'select' && isDragging.value) return; // Prevent selection if dragging in pan mode
...
const getEdgeColor = (id) => {
  ...
  if (util > 0.9) return '#e74c3c'; // Red
  if (util >= 0.75) return '#f39c12'; // Orange
  return null;
};
