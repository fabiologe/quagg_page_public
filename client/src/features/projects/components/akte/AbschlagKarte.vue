<template>
  <ProjektKarte titel="Abschlag aus Leistungsstand" icon="stellen">
    ...
    <template v-else>
      <p v-if="vorschau && !vorschau.positionen.length" class="prj-hinweis">Nichts fällig — der Leistungsstand ist vollständig abgerechnet.</p>
      <template v-else-if="vorschau">
        <p class="prj-hinweis">{{ vorschau.nummer }}. Abschlag, netto <strong>{{ centAlsEuro(vorschau.summe_netto_cent) }}</strong> — je Abschnitt kumulierte Leistung abzüglich bereits gestellter Positionen:</p>
        <ul class="prj-positionen">
          <li v-for="p in vorschau.positionen" :key="p.abschnitt_id">
            <span>{{ p.bezeichnung }}</span>
            <strong>{{ centAlsEuro(p.einzelpreis_cent) }}</strong>
          </li>
        </ul>
        <form class="prj-zeile" @submit.prevent="anlegen">
          ...
          <button type="submit" class="prj-knopf prj-knopf-primaer" :disabled="!von || laeuft">
            <ProjektIcon :name="laeuft ? 'laden' : 'stellen'" :size="14" /> Entwurf im Pedanten anlegen
          </button>
        </form>
      </template>
...
<script setup>
// AbschlagKarte — Vorschau der fälligen Positionen, dann Rechnungs-ENTWURF im Pedanten.
// Stellen (Nummer, XRechnung, KoSIT) bleibt dort — hier wird nichts unwiderruflich.
...
async function laden() {
  ...
  try {
    vorschau.value = await ProjekteApi.abschlagVorschau(props.akte.id);
  } catch (error) {
    console.warn('projekte abschlag:', error);
    vorschau.value = null;
  }
}
...
async function anlegen() {
  laeuft.value = true;
  try {
    ergebnis.value = await store.abschlagAnlegen(props.akte.id, { leistung_von: von.value, leistung_bis: bis.value || null });
...
.prj-positionen { list-style: none; margin: 0 0 0.8rem; padding: 0; }
