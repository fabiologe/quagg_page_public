  ...
  <div class="custom-select" :class="{ 'open': isOpen }" ref="container">
    <div class="select-trigger" @click="toggle" :class="{ 'has-selection': selectedOption }">
      <div v-if="selectedOption" class="selected-content">
        ...
        <span class="name">{{ selectedOption.id }} – {{ selectedOption.name }}</span>
        <span class="cat-badge" :style="{ backgroundColor: selectedOption.color }">Kat. {{ selectedOption.category }}</span>
      </div>
      <span v-else class="placeholder">Flächengruppe wählen...</span>
      ...
      <div v-if="isOpen" class="options-list">
        ...
        >
          ...
          <div class="option-info">
            <span class="name"><strong>{{ option.id }}</strong> – {{ option.name }}</span>
            <span class="sub-info">Kat. {{ option.category }} ({{ option.categoryLabel }}) · {{ option.specificLoad }} kg/(ha·a)</span>
          </div>
...
const props = defineProps({
  ...
  options: {
    type: Array, // Erwartet: [{ id, name, category, categoryLabel, specificLoad, color }, ...]
    default: () => []
  }
})
