...
void fileoutput(Fnames *Fnameptr,States *Statesptr,Pars *Parptr,Arrays *Arrptr)
{
  ...
  {	  
	// Write time of initial flood inundation
	write_ascfile(Fnameptr->resrootname,-1,(char*)".inittm",Arrptr->initHtm,Arrptr->DEM,0,Statesptr,Parptr);
	// Write total inundation time
	write_ascfile(Fnameptr->resrootname,-1,(char*)".totaltm",Arrptr->totalHtm,Arrptr->DEM,0,Statesptr,Parptr);
	// Write maximum depth
	write_ascfile(Fnameptr->resrootname,-1,(char*)".max",Arrptr->maxH,Arrptr->DEM,0,Statesptr,Parptr);
	// Write time of maximum depth
	write_ascfile(Fnameptr->resrootname,-1,(char*)".maxtm",Arrptr->maxHtm,Arrptr->DEM,0,Statesptr,Parptr);
	if(Statesptr->voutput==ON)
	{
		write_ascfile(Fnameptr->resrootname,-1,(char*)".maxVx",Arrptr->maxVx,Arrptr->DEM,1,Statesptr,Parptr);
		write_ascfile(Fnameptr->resrootname,-1,(char*)".maxVy",Arrptr->maxVy,Arrptr->DEM,2,Statesptr,Parptr);
	}
	if(Statesptr->hazard==ON)
	{
		// Write maximum V Vd and Hazard
		write_ascfile(Fnameptr->resrootname,-1,(char*)".maxVc",Arrptr->maxVc,Arrptr->DEM,0,Statesptr,Parptr);
		write_ascfile(Fnameptr->resrootname,-1,(char*)".maxVcd",Arrptr->maxVcH,Arrptr->DEM,0,Statesptr,Parptr);
		write_ascfile(Fnameptr->resrootname,-1,(char*)".maxHaz",Arrptr->maxHaz,Arrptr->DEM,0,Statesptr,Parptr);
	}
  }
}
...
void debugfileoutput(Fnames *Fnameptr,States *Statesptr,Pars *Parptr,Arrays *Arrptr)
{
  ...
  write_ascfile(Fnameptr->resrootname,-1,(char*)".dem",Arrptr->DEM,Arrptr->DEM,0,Statesptr,Parptr);
  ...
  if(Statesptr->ChannelPresent==ON) 
  {
    ...
    write_ascfile(Fnameptr->resrootname,-1,(char*)".chmask",TempChanMask,Arrptr->DEM,0,Statesptr,Parptr);
    ...
    write_ascfile(Fnameptr->resrootname,-1,(char*)".segmask",TempChanMask,Arrptr->DEM,0,Statesptr,Parptr);
  ...
  if(Statesptr->SGC==ON) 
  {
	...
	write_ascfile(Fnameptr->resrootname,-1,(char*)"_SGC_bedZ.asc",Arrptr->SGCz,Arrptr->DEM,0,Statesptr,Parptr);
	// Write out width
	write_ascfile(Fnameptr->resrootname,-1,(char*)"_SGC_width.asc",Arrptr->SGCwidth,Arrptr->DEM,0,Statesptr,Parptr);
	// Write out bankfull depth
	write_ascfile(Fnameptr->resrootname,-1,(char*)"_SGC_bfdepth.asc",Arrptr->SGCbfH,Arrptr->DEM,0,Statesptr,Parptr);
  }
...
void write_regular_output( Fnames *Fnameptr, Solver *Solverptr, States *Statesptr, Pars *Parptr, Arrays *Arrptr, SGCprams *SGCptr  )
{
    ...
    if(Statesptr->save_depth==ON)
    {
		...
		if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".wdb",Arrptr->H,Arrptr->DEM,0,Statesptr,Parptr);
		else write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".wd",Arrptr->H,Arrptr->DEM,0,Statesptr,Parptr);
		if (Statesptr->SGC==ON)
		{
			// export floodplain depth for SGC model only
			if (Statesptr->binary_out==ON) write_binrasterfile_SGCf(Fnameptr->resrootname,Parptr->SaveNo, (char*)".wdfpd", Arrptr->H, Arrptr->SGCbfH, Statesptr,Parptr); 
			else write_ascfile_SGCf(Fnameptr->resrootname,Parptr->SaveNo, (char*)".wdfp", Arrptr->H, Arrptr->SGCbfH, Statesptr,Parptr);
		}
	}
    ...
    if(Statesptr->save_elev==ON)
    {
	  ...
	  if (Statesptr->SGC==ON)
	  {
		  if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".elevb",Arrptr->H,Arrptr->SGCz,3,Statesptr,Parptr);
		  else write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".elev",Arrptr->H,Arrptr->SGCz,3,Statesptr,Parptr);
	  }
	  else
	  {
		  // output binary of ascii rasters
		  if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".elevb",Arrptr->H,Arrptr->DEM,3,Statesptr,Parptr);
		  else write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".elev",Arrptr->H,Arrptr->DEM,3,Statesptr,Parptr);
	  }
	}
    ...
    if(Statesptr->save_Qs==ON)
	{
		...
		{
			write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".Qx",Arrptr->Qx,Arrptr->DEM,1,Statesptr,Parptr);
			write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".Qy",Arrptr->Qy,Arrptr->DEM,2,Statesptr,Parptr);
		}
		if (Statesptr->SGC==ON)
		{
			...
			{
				write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".Fwidth",Arrptr->SGCFlowWidth,Arrptr->DEM,0,Statesptr,Parptr);
				// channel Q's
				write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".Qcx",Arrptr->QxSGold,Arrptr->DEM,1,Statesptr,Parptr);
				write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".Qcy",Arrptr->QySGold,Arrptr->DEM,2,Statesptr,Parptr);
			}
		}
	}
    ...
    if(Statesptr->voutput==ON)
    {
		...
		{
			write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".Vx",Arrptr->Vx,Arrptr->DEM,1,Statesptr,Parptr);
			write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".Vy",Arrptr->Vy,Arrptr->DEM,2,Statesptr,Parptr);
		}
	}
	...
	if (Statesptr->SGC==ON && Statesptr->SGCvoutput == ON)
	{
		...
		{
			write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".SGCVx",SGCVx,Arrptr->DEM,1,Statesptr,Parptr);
			write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".SGCVy",SGCVy,Arrptr->DEM,2,Statesptr,Parptr);
			write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".SGCVc",SGCvoutput,Arrptr->DEM,0,Statesptr,Parptr);
		}
    ...
    if(Statesptr->save_QLs==ON)
    {
		...
		{
			write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".QLx",Arrptr->LimQx,Arrptr->DEM,1,Statesptr,Parptr);
			write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,(char*)".QLy",Arrptr->LimQy,Arrptr->DEM,2,Statesptr,Parptr);
		}
	}
