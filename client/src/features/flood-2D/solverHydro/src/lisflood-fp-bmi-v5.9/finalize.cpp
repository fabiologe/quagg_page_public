...
void final_iterateq()
{
  ...
  if (Statesptr->SGC==ON) // SGC output
  {
	...
	if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,-1,(char*)".mxeb",Arrptr->maxH,Arrptr->SGCz,0,Statesptr,Parptr);
	else write_ascfile(Fnameptr->resrootname,-1,(char*)".mxe",Arrptr->maxH,Arrptr->SGCz,0,Statesptr,Parptr);
  }
  else
  {
	for(int i=0;i<Parptr->xsz;i++) for(int j=0;j<Parptr->ysz;j++)
	...
	if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,-1,(char*)".mxeb",Arrptr->maxH,Arrptr->DEM,0,Statesptr,Parptr);
	else write_ascfile(Fnameptr->resrootname,-1,(char*)".mxe",Arrptr->maxH,Arrptr->DEM,0,Statesptr,Parptr);
  }
