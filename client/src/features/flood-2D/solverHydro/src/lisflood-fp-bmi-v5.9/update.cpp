...
#pragma omp parallel for private(j,ptr) reduction ( + : FloodArea)
  ...
  if(Solverptr->t>=Parptr->SaveTotal)
  {
    ...
    {
      // write rasters
	  if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,-1,(char*)".opb",Arrptr->H,Arrptr->DEM,0,Statesptr,Parptr);
      else write_ascfile(Fnameptr->resrootname,-1,(char*)".op",Arrptr->H,Arrptr->DEM,0,Statesptr,Parptr);
      // write profiles
    ...
    if(Statesptr->save_elev==ON)
		{
		  ...
		  {
			if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,-1,(char*)".opelevb",Arrptr->H,Arrptr->SGCz,3,Statesptr,Parptr);
			else write_ascfile(Fnameptr->resrootname,-1,(char*)".opelev",Arrptr->H,Arrptr->SGCz,3,Statesptr,Parptr);
		  }
		  else // standard model output
		  {
			if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,-1,(char*)".opelevb",Arrptr->H,Arrptr->DEM,3,Statesptr,Parptr);
			else write_ascfile(Fnameptr->resrootname,-1,(char*)".opelev",Arrptr->H,Arrptr->DEM,3,Statesptr,Parptr);
		  }
		}
	}
  ...
  if(Statesptr->multi_op==ON)
  {
    for(i=0;i<Parptr->op_multinum;i++)
    {
      ...
      {
		  // raster depth output
          if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,i,(char*)"-T.opb",Arrptr->H,Arrptr->DEM,0,Statesptr,Parptr);
		  else write_ascfile(Fnameptr->resrootname,i,(char*)"-T.op",Arrptr->H,Arrptr->DEM,0,Statesptr,Parptr);
		  // profiles output
		  if (Statesptr->profileoutput==ON) write_profile(Fnameptr->resrootname,i,(char*)"-T.profile",Statesptr,ChannelSegments,Arrptr,Parptr, RiversIndexVecPtr, RiversIndexPtr);
        ...
        if(*verbose==ON) printf("Writing overpass %d at %lf seconds\n",i,Parptr->op_multisteps[i]);
		  ...
		  if(Statesptr->save_elev==ON)
	      {
			  if (Statesptr->SGC==ON) // SGC output
			  {
				if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,i,(char*)"-T.opelevb",Arrptr->H,Arrptr->SGCz,3,Statesptr,Parptr);
				else write_ascfile(Fnameptr->resrootname,i,(char*)"-T.opelev",Arrptr->H,Arrptr->SGCz,3,Statesptr,Parptr);
			  }
			  else // standard model output
			  {
				if (Statesptr->binary_out==ON) write_binrasterfile(Fnameptr->resrootname,i,(char*)"-T.opelevb",Arrptr->H,Arrptr->DEM,3,Statesptr,Parptr);
				else write_ascfile(Fnameptr->resrootname,i,(char*)"-T.opelev",Arrptr->H,Arrptr->DEM,3,Statesptr,Parptr);
			  }
		  }
      }
    }
