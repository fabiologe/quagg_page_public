...
export function exportPlanPDF({ snapshot, format, orientation, titleBlock, logo, dimensions = [] }) {
    ...
    // ── Title block ───────────────────────────────────────────────────
    const ty = M + dh;  // top of title block

    doc.setLineWidth(0.3);
    doc.line(M, ty, M + dw, ty);           // title block top line
    doc.line(M, ty + 20, M + dw, ty + 20); // mid horizontal divider

    // Vertical column dividers at 40%, 65%, 82%
    const cols = [0.40, 0.65, 0.82];
    cols.forEach(f => doc.line(M + dw * f, ty, M + dw * f, ty + TB));

    // ── Row 1: Projekt | Auftraggeber | Bearbeiter | Logo/Firma ──────
    const row1 = [
        ['Projekt',       titleBlock.projekt       ?? '', 0],
        ['Auftraggeber',  titleBlock.auftraggeber  ?? '', 0.40],
        ['Bearbeiter',    titleBlock.bearbeiter    ?? '', 0.65],
    ];
    row1.forEach(([lbl, val, f]) => {
        doc.setFontSize(6);  doc.setTextColor(130); doc.text(lbl, M + dw * f + 1.5, ty + 4);
        doc.setFontSize(9);  doc.setTextColor(0);   doc.text(val, M + dw * f + 1.5, ty + 11, { maxWidth: dw * (f === 0 ? 0.39 : 0.24) - 3 });
    });
    ...
    // ── Row 2: Nr. | Datum | Maßstab + Index | (logo continues) ─────
    const defaultDate = new Date().toLocaleDateString('de-DE');
    const row2 = [
        ['Nr.',      titleBlock.nummer    ?? '', 0],
        ['Datum',    titleBlock.datum     || defaultDate, 0.40],
        ['Maßstab',  titleBlock.massstab  ?? '1:100', 0.65],
    ];
    row2.forEach(([lbl, val, f]) => {
        doc.setFontSize(6);  doc.setTextColor(130); doc.text(lbl, M + dw * f + 1.5, ty + 24);
        doc.setFontSize(9);  doc.setTextColor(0);   doc.text(val, M + dw * f + 1.5, ty + 31, { maxWidth: dw * (f === 0 ? 0.39 : 0.24) - 3 });
    });

    // Index in Maßstab-cell bottom-right corner
    if (titleBlock.index) {
...
export async function exportVectorPlanPDF({
    ...
    if (categoryGroups && fragmentsList) {
        try {
            outlines = await extractConcaveOutlines(categoryGroups, fragmentsList, {
                viewDir, rules, fragmentsManager,
            });
        } catch (e) {
    ...
    // ── Outer border + title block (reuse existing logic) ────────────
    doc.setLineWidth(0.7);
    doc.setDrawColor(0);
    doc.rect(M, M, dw, dh + TB);
